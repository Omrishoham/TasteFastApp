...
public class AppController implements PropertyChangeListener {
	...
	public void propertyChange(PropertyChangeEvent event) {
		if (event.getPropertyName().equals("SignUpClientEvent")) {
			if (!(model.ifClientExist(((Client) event.getNewValue()).getUsername(),
					((Client) event.getNewValue()).getEmail()))) {
			...
			if (model.loginClientAuth(((Client) event.getNewValue()).getUsername(),
					((Client) event.getNewValue()).getPassword())) {
		...
		// employee
			...
			if (model.loginEmployeeAuth(((Employee) event.getNewValue()).getUsername(),
					((Employee) event.getNewValue()).getPassword())) {
				// check if is a manager, and wither way place the other values to object
				// employee
				if (model.ifEmployeeManager(((Employee) event.getNewValue()).getUsername(),
						((Employee) event.getNewValue()).getPassword())) {
					Employee employee = model.placeValues(((Employee) event.getNewValue()).getUsername(),
							((Employee) event.getNewValue()).getPassword());
					Manager manager = new Manager(employee.getUsername(), employee.getPassword(),
							employee.getSalaryPerHour(), employee.getSalarySum(), employee.getFirstName(),
							employee.getLastName());

					view.changeWindows("ManagerPanel", (Object) manager);
				} else {
					Employee employee1 = model.placeValues(((Employee) event.getNewValue()).getUsername(),
							((Employee) event.getNewValue()).getPassword());
					Waiter waiter = new Waiter(employee1.getUsername(), employee1.getPassword(),
							employee1.getSalaryPerHour(), employee1.getSalarySum(), employee1.getFirstName(),
			...
			if (model.ifEmployeeExist(((Employee) event.getNewValue()).getUsername())) {
				model.updateToManager(((Employee) event.getNewValue()).getUsername(),
